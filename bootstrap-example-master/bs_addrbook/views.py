...
from forms import RegistrationForm, CreateForm, EditForm
...
def search(request):
    if not 'last' in request.GET:
        return render(request, 'bs_addrbook/search.html', {})
    ...
    if objects.count() == 0:
        ...
        return render(request, 'bs_addrbook/search.html', {'message': message})

    if objects.count() > 1:
        context = { 'entries': objects.order_by('last_name', 'first_name') }
        return render(request, 'bs_addrbook/list.html', context)

    entry = objects.all()[0]
    form = EditForm(instance=entry)
    context = { 'entry': entry, 'form': form }
    return render(request, 'bs_addrbook/edit.html', context)

@login_required
def create(request):
    if request.method == 'GET':
        context = { 'form': CreateForm() }
        return render(request, 'bs_addrbook/create.html', context)
...
def edit(request, id):
    try:
        ...
        return render(request, 'bs_addrbook/search.html', context)
...
def register(request):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'bs_addrbook/register.html', context)
