...
def search(request):
    ...
    if objects.count() == 0:
        message = ugettext('No entries with last name = {0}').format(last)
...
def create(request):
    ...
    message = ugettext('Entry created')
...
def delete(request, id):
    if request.method != 'POST':
        message = ugettext('Invalid request.  POST method must be used.')
        return render(request, 'addrbook/search.html', { 'message': message })

    entry = get_object_or_404(Entry, id=id)
    entry.delete()
    message = ugettext('Entry for {0}, {1} has been deleted.').format(entry.last_name, entry.first_name)
    return render(request, 'addrbook/search.html', { 'message': message })
...
def edit(request, id):
    try:
        ...
        # form = EditForm(instance=entry)
        context = {
            'message': ugettext('Entry updated.'),
            'entry':   entry,
            'form':    EditForm(instance=entry),
        }
        return render(request, 'addrbook/edit.html', context)
